...
// https://github.com/buxtronix/golang/blob/master/nmea/src/nmea/latlong.go
...
func NewLatLong(s string) (LatLong, error) {
	...
	if err == nil {
		...
		slog.Error("parseDMS", slog.Any("err", err))
	}
	...
	if err == nil {
		...
		slog.Error("parseGPS", slog.Any("err", err))
	}
	...
	if err == nil {
		...
		slog.Error("parseDecimal", slog.Any("err", err))
	}
...
func ParseGPS(s string) (LatLong, error) {
	...
	if len(parts) < 2 {
		return -1, fmt.Errorf("parseGPS failed. len not expected s: %s", s)
	}
	dir := parts[1]
